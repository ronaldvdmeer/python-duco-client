...
|---|---|---|
| `api_version` | `str` | Backward-compatible alias for `public_api_version` |
| `public_api_version` | `str` | Public API version (e.g. `"2.5"`) |
| `reported_api_version` | `str \| None` | Optional additional API version string when the device reports `ApiVersion` |
| `endpoints` | `list[ApiEndpointInfo]` | Endpoint metadata returned by `/api` |

### `ApiEndpointInfo`

| Field | Type | Description |
|---|---|---|
| `url` | `str` | API path (e.g. `"/info"`) |
| `query_parameters` | `list[str]` | Supported query parameters |
| `methods` | `list[str]` | Supported HTTP methods |
| `modules` | `list[str]` | Supported filter modules when present |
...
Get board identification and version metadata.
...
| `public_api_version` | `str \| None` | Public API version reported in board metadata |
| `software_version` | `str \| None` | Optional board software version when reported by the firmware |
